            ...
            if opinions_count:
                ...
                while (url):
                    ...
                    product = {
                        ...
                        'recommendation_distribution' : opinions.rating.value_counts(dropna=False).reindex(["Polecam","Nie polecam", None]).to_dict(),
...
def download_csv(product_id):
    ...
    buffor=io.BytesIO(opinions.to_csv(sep=";",decimal=",").encode())
    
    return send_file(buffor,"text/csv", as_attachment=True, download_name=f"{product_id}.csv")

@app.route("/product/download_xlsx/<product_id>")
def download_xlsx(product_id):
    return send_file(f"data/opinions/{product_id}.json","text/xlsx", as_attachment=True)